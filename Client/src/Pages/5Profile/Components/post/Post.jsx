...
const Post = ({post, userId, sendNewPost, refreshPosts }) => {
  ...
  const likeHandler = async () => {
    await axios.patch(`${process.env.REACT_APP_BACKEND_LINK}/likes/${post._id}`, { userId }, { withCredentials: true })
  ...
  const commentPost = () => {
    ...
    if (comment) {
      ...
      axios.post(`${process.env.REACT_APP_BACKEND_LINK}/comments/${userId}`, formData, { withCredentials: true })
        .then((response) => { return response })
        .then(({ data }) => {
          console.log(data)
  ...
  const commentGet = () => {
    ...
    axios.get(`${process.env.REACT_APP_BACKEND_LINK}/posts/comments/${postId}`, { withCredentials: true })
      .then((response) => { return response })
      .then(({ data }) => {
        setComments(data)
        console.log(data)
  ...
  const DeleteMyPost = () => {
    const id = post._id
    axios.delete(`${process.env.REACT_APP_BACKEND_LINK}/posts/${id}`, { withCredentials: true })
      .then((response) => { return response })
      .then(({ data }) => {
        console.log(data)
        refreshPosts()
  ...
  useEffect(() => {
    ...
    axios.get(`${process.env.REACT_APP_BACKEND_LINK}/user/${id}`, { withCredentials: true })
      .then((response) => { return response })
      .then(({ data }) => {
        setOtherUser(data)
      })
      ...
      <div className='post'>
        <div className="postWrapper">
          <div className="postTop">
            ...
            <div className="postTopLeft">
              <NavLink to={`/profile`} state={{ user: otherUser }}> 
                <img className='postProfileImg'  src={`${process.env.REACT_APP_BACKEND_LINK}/images/${post.userPicturePath}`} alt="" />
              </NavLink>
          ...
          <div className="postCenter">
            ...
            {post.picturePath && <img className='postImg' src={`${process.env.REACT_APP_BACKEND_LINK}/images/${post.picturePath}`} alt="" />}
          </div>
